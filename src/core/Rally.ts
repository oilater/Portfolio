...
import { DEFAULTS, type AddMotionsProps, type ElementType, type GetMotionTlProps, type Motion, type MotionValueType } from "./types";
...
export function useRally() {
  ...
  function Rally({
    ...
    addMotions({tl: rallyTl, elements, motions, mode: "enter", splitDelay});
    
    if (exitMotions) {
      const exitTl = gsap.timeline({ paused: true });
      addMotions({tl: exitTl, elements, motions: exitMotions, mode: "exit"});
  ...
  function addMotions({tl, elements, motions, mode, splitDelay}: AddMotionsProps): void {
